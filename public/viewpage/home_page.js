...
export async function home_page() {
  ...
  for (let i = 0; i < decForms.length; i++) {
    decForms[i].addEventListener("submit", (e) => {
      ...
      // add (decrement) p to shopping cart
      cart.removeItem(p);
  ...
  for (let i = 0; i < incForms.length; i++) {
    incForms[i].addEventListener("submit", (e) => {
      e.preventDefault();
      const p = products[e.target.index.value];
      // add (increment) p to shopping cart
      cart.addItem(p);
...
function buildProductView(product, index) {
  return `
    ...
    `; /* Auth.currentUser ? d-block : d-none ==> if the current user is not null, (means a user has 
       signed in) then that div tag will be rendered. That div tag will contain the buttons and stuff about 
       how many items you want in the cart and stuff like that, which you can only do if you're signed in. */
}
